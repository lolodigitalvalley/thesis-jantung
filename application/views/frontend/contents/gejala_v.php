    ...
    <div class="col-md-12">
         <div class="box-body">
            <div class="form-group">
                <button type="button" onClick="add_gejala();" class="btn btn-primary" id="add-gejala" >Tambah</button><br/><br/> 
                <div class="table-responsive" id="table-responsive">
                <?php echo !empty($data_gejala) ? $data_gejala : ''; ?>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function add_gejala()
    {
        ...
        $('.modal-title').text('Tambah Gejala'); // Set Title to Bootstrap modal title
    }

    function update_gejala(kode_gejala)
    {
         $.ajax({
            url: baseUrl + "update_gejala/",
        ...
        $('#modal-form').modal('show'); // show bootstrap modal
        $('.modal-title').text('Update Gejala'); // Set Title to Bootstrap modal title
    }

    function delete_gejala(kode_gejala)
    {
        if(confirm('Are you sure delete this data?'))
        {
            $.ajax({
                url : baseUrl + "delete_gejala/",
                data: {'kode_gejala':kode_gejala},
                type: "POST",
                dataType: "JSON",
                success: function(data)
    ...
    function save_gejala()
    {
        ...
        $.ajax({
            url : baseUrl + "save_gejala/",
            type: "POST",
            data: new FormData($('#form-gejala')[0]),
            dataType: "JSON",
