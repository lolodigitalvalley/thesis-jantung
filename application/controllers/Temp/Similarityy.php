...
class Similarity extends CI_Controller
{
    ...
    public function retrive_with_indexing()
    {
        ...
        $jt            = count($gejala_baru) + 2;
        for ($p = 1; $p <= $np; $p++) {
            $knn             = new knn($centroids, $newCase[$p], 1, 'squaredEuclidean');
            //$knn             = new knn($centroids, $newCase[$p], 1, 'cosinus');
            $nearestCluster  = $knn->get_nn() + 1;
            $nearestDistance = $knn->get_distance();

            if (!in_array($nearestCluster, $klaster_list)) {
                $klaster_list[] = $nearestCluster;
        ...
        $json['nearestCase']  = count($klaster_list);
