...
class Similarity extends CI_Controller
{
    ...
    public function retrive_non_indexing()
    {
        ...
        if ($sim == 'K_NN') {
            ...
            for ($i = 1, $j = 0; $i < 56; $i++) {
                if (in_array($i, $index)) {
                    $newCase[1][$i+1] = $rows[$j]->p1;
                    $newCase[2][$i+1] = $rows[$j]->p2;
                    $newCase[3][$i+1] = $rows[$j]->p3;
                    $newCase[4][$i+1] = $rows[$j]->p4;
                    $newCase[5][$i+1] = $rows[$j]->p5;
                    $newCase[6][$i+1] = $rows[$j]->p6;
                    $j++;
                } else {
                    $newCase[1][$i+1] = 0;
                    $newCase[2][$i+1] = 0;
        ...
        if ($sim == 'K_NN') {
            $knn             = new knn($oldCase, $newCase[1], 1, 'squaredEuclidean');
            $index           = $knn->get_nn();
            $nearestDistance = $knn->get_distance();

            for ($i = 2; $i < 7; $i++) {
                $knn = new knn($oldCase, $newCase[$i], 1, 'squaredEuclidean');
    ...
    public function retrive_with_indexing()
    {
        ...
        $nearestClusterx  = $knn->get_nn() + 1;
        $nearestDistancex = $knn->get_distance();
        $p = 1;
        for ($i = 2; $i < 7; $i++) {
            $knn = new knn($centroids, $newCase[$i], 1);
            if ($knn->get_distance() < $nearestDistancex) {
                $nearestClusterx  = $knn->get_nn() + 1;
                $nearestDistancex = $knn->get_distance();
                $p               = $i;
            }
        ...
        if($nearestClusterx < $nearestCluster) $cluster = $nearestClusterx;
        else  $cluster = $nearestCluster;
        */
        
        $result = $this->get_similarity($newCase[0], $indexing, $sim, $nearestCluster);        

        $this->benchmark->mark('code_end');
        
        $result['elapsed_time'] = $this->benchmark->elapsed_time('code_start', 'code_end');

        echo json_encode($result);
    }

    private function get_similarity($newCase, $indexing = 'som', $sim, $nearestCluster){
            
            $fields = 'kode_kasus, kode_penyakit, jk, umur, g1, g2, g3, g4, g5, g6, g7, g8, g9, g10, g11, g12, g13, g14, g15, g16, g17, g18, g19, g20,
                     g21, g22, g23, g24, g25, g26, g27, g28, g29, g30,g31, g32, g33, g34, g35, g36, g37, g38, g39, g40,
                     g41, g42, g43, g44, g45,r1, r2, r3, r4, r5, r6, r7, r8, r9, r10 ';
            ...
            if ($sim == 'K_NN') {
                $knn             = new knn($oldCase, $newCase, 1, 'cosinus');
                $index           = $knn->get_nn();
                $nearestDistance = $knn->get_distance();
